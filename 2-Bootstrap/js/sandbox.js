...
let max = 100; // max is a variable that will store the value of 100;
let min = 1; // min will store 1 as its value; 
let nGuesses = 0; // Everytime the function "Guesses", it'll be stored in this variable
let currGuess = 0; // This is the
...
const resBtn = document.getElementById("resBtn"); // Reset Button to start the Game all over
...
function isGuessCorrect(correct) {
    ...
    if (correct) {
        ...
        toggleBtns([resBtn], true); // add the restart button if the function guessed correctly (yes is clicked)
